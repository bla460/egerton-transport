document.addEventListener('DOMContentLoaded', () => {
  ...
  function applyStatusToCard(card, status) {
    ...
    // Find existing status span or paragraph
    let statusEl = card.querySelector('.status-pill') || card.querySelector('p span') || null;
    if (!statusEl) {
      // create a consistent status element
      statusEl = document.createElement('span');
      statusEl.className = 'status-pill';
      const p = card.querySelector('p') || document.createElement('p');
      p.textContent = 'Status: ';
      p.appendChild(statusEl);
      if (!card.contains(p)) card.appendChild(p);
    }
  ...
  function renderFleetStatuses() {
    ...
    document.querySelectorAll('.vehicle-card').forEach((card) => {
      // try to find vehicle id from link
  ...
  // Re-render when admin updates fleet
  window.addEventListener('egerton-fleet-updated', () => {
    setTimeout(renderFleetStatuses, 40);
  });
